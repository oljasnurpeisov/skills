...
@section('title',__('default.pages.task.title').' ID: '.$item->name.' | '.__('default.site_name'))
...
@section('content')
    <div class="container container-fluid">
        <ul class="breadcrumbs">
            ...
            <li class="active">{{ __('default.pages.task.title') }} ID: {{ $item->id }}</li>
        </ul>
        @include('admin.v2.partials.components.warning')
        @include('admin.v2.partials.components.errors')
        <form action="/admin/task/{{ $item->id }}" method="post"
                ...
                <div class="input-group {{ $errors->has('producer_id') ? ' has-error' : '' }}">
                    ...
                    <select name="fridge_id" id="fridge_id" class="input-regular chosen" data-placeholder=" " required>
                        ...
                        @foreach($fridges as $fridge)
                            ...
                            @if($item->fridge_id === $fridge->id)
                                @php($availableDishes = $fridge->dishes->pluck('id')->toArray())
                            @endif
                        @endforeach
                    </select>
                    ...
                    <div class="block">
                        ...
                        <table class="table records">
                            ...
                            @foreach($item->dishes as $key => $dish)
                                @if($dish->pivot->type === 'unload' && in_array($dish->id,$availableDishes,true))
                                    <tr>
                                        <td>
            ...
            <div class="block">
                <div class="buttons">
                    ...
                    <div>
                        <a href="/admin/task/{{ $item->id }}/export.pdf" class="btn btn--blue">
                            {{ __('default.pages.task.export') }}
                        </a>
                    </div>
                    <div>
                        <a href="/admin/task/{{ $item->id }}" class="btn btn--red btn--delete">
                    ...
                    <select name="dishes[unload][ids][]" class="input-regular" required>
                        ...
                        @foreach($dishes as $dish)
                            <option value="{{ $dish->id }}"
                                    @if(!in_array($dish->id,$availableDishes,true)) disabled @endif>{{ $dish->name }}</option>
                        @endforeach
                    </select>
                </td>
                <td>
